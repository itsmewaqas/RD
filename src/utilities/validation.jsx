function validateInfo({ values }) {
    ...
    if (values.description !== null) {
        if (!values.description) {
            errors.description = "please inter your description";
        }
    }

    if (values.folderName !== null) {
        if (!values.folderName) {
            errors.folderName = "please inter your folder name";
        } else if (values.folderName.length < 6) {
            errors.folderName = "name need to be 6 characters or more'";
        }
    }
    ...
    if (values.format !== null) {
        if (!values.format) {
            errors.format = "Please select format";
        }
    }

    if (values.encoding !== null) {
        if (!values.encoding) {
            errors.encoding = "Please select encoding";
        }
    }
